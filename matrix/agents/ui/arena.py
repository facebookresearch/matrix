...
if "scores" not in st.session_state:
    st.session_state.scores = {"Alpha": {"iq": 50, "eq": 50}, "Beta": {"iq": 50, "eq": 50}, "Gamma": {"iq": 50, "eq": 50}}
...
# ── Debate formats ────────────────────────────────────────────────────────────
FORMATS = {
    "Quick Round-Robin (12 turns, ~12 min)": {"turns": 12, "label": "T1", "color": "#4f8ef7"},
    "Oxford Modified (18 turns, ~22 min)":   {"turns": 18, "label": "T2", "color": "#f7a04f"},
    "Extended Cross-Exam (30 turns, ~38 min)": {"turns": 30, "label": "T3", "color": "#f74f8e"},
}
...
with st.sidebar:
    ...
    st.markdown('<div class="section-header">Debate Topic</div>', unsafe_allow_html=True)
    topic_choice = st.selectbox("Choose a sample topic or enter custom:", ["Custom..."] + SAMPLE_TOPICS)
    if topic_choice == "Custom...":
        topic = st.text_input("Enter your debate topic:", placeholder="e.g. AI should replace human judges")
    else:
        topic = topic_choice
    st.session_state.topic = topic

    st.markdown('<div class="section-header">Debate Format</div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="section-header">Agent Models</div>', unsafe_allow_html=True)
    ...
    if human_enabled:
        human_position = st.radio("Your position:", ["PRO", "CON", "NEUTRAL", "MODERATOR"])
...
# ── Main area ─────────────────────────────────────────────────────────────────
st.markdown('<div class="main-title">🎙️ Sokrates Speech LLM Arena</div>', unsafe_allow_html=True)
st.markdown('<div class="subtitle">Three-dimensional evaluation via multi-agent debate · IQ · EQ · Joint Score</div>', unsafe_allow_html=True)

# ── Tabs ──────────────────────────────────────────────────────────────────────
tab_arena, tab_scores, tab_voice, tab_rollouts, tab_about = st.tabs(["🏛️ Arena", "📊 Scores", "🎤 Voice Input", "🎧 Rollouts", "ℹ️ About"])
...
with tab_arena:
    ...
    progress_val = st.session_state.turn_count / total_turns if total_turns > 0 else 0
    col_status1, col_status2, col_status3 = st.columns([3, 1, 1])
    with col_status1:
        st.progress(progress_val, text=f"Turn {st.session_state.turn_count} / {total_turns}")
    with col_status2:
        status_icon = "🟢 Running" if st.session_state.debate_running else ("✅ Finished" if st.session_state.debate_finished else "⏸ Idle")
    ...
    with col_a:
        st.markdown(f"<div style='color:#4f8ef7;font-weight:700;font-size:1.1rem;'>🔵 Alpha — PRO</div>", unsafe_allow_html=True)
        st.caption(f"Model: {model_alpha}")
    with col_b:
        st.markdown(f"<div style='color:#f74f8e;font-weight:700;font-size:1.1rem;'>🔴 Beta — CON</div>", unsafe_allow_html=True)
        st.caption(f"Model: {model_beta}")
    with col_g:
        st.markdown(f"<div style='color:#4ff7a0;font-weight:700;font-size:1.1rem;'>🟢 Gamma — NEUTRAL</div>", unsafe_allow_html=True)
...
Now deliver your next argument."""
    ...
    def update_scores(agent_name):
        ...
        st.session_state.scores[agent_name]["iq"] = min(100, max(0, st.session_state.scores[agent_name]["iq"] + delta_iq))
        st.session_state.scores[agent_name]["eq"] = min(100, max(0, st.session_state.scores[agent_name]["eq"] + delta_eq))
    ...
    if start_btn and topic:
        ...
        for turn in range(1, total_turns + 1):
            ...
            with debate_container:
                ...
                full_text = ""
                for chunk in stream_agent_response(agent_name, position, model, topic, st.session_state.debate_history):
                    full_text += chunk
...
with tab_scores:
    ...
    st.caption("Scores update after each turn. IQ = argument quality (LLM judge on transcript). EQ = prosodic delivery (simulated).")
    ...
    if not st.session_state.debate_history:
        ...
        for col, (agent, color) in zip([col1, col2, col3], [("Alpha", "#4f8ef7"), ("Beta", "#f74f8e"), ("Gamma", "#4ff7a0")]):
            with col:
                iq = scores[agent]["iq"]
                eq = scores[agent]["eq"]
                joint = round(iq * iq_weight + eq * eq_weight)
                st.markdown(f"<div style='color:{color};font-weight:700;font-size:1.1rem;margin-bottom:0.5rem;'>{agent}</div>", unsafe_allow_html=True)

                st.markdown(f"**IQ Score** (what was said)")
                st.progress(iq / 100)
                st.markdown(f"<div style='font-size:1.8rem;font-weight:700;color:{color};'>{iq}</div>", unsafe_allow_html=True)

                st.markdown(f"**EQ Score** (how it was said)")
                st.progress(eq / 100)
                st.markdown(f"<div style='font-size:1.8rem;font-weight:700;color:{color};'>{eq}</div>", unsafe_allow_html=True)

                st.markdown(f"**Joint Score** (IQ–EQ alignment)")
                st.progress(joint / 100)
                st.markdown(f"<div style='font-size:2rem;font-weight:700;color:{color};'>{joint} 🏆</div>", unsafe_allow_html=True)
        ...
        for entry in st.session_state.debate_history:
            ...
            st.markdown(f"**Turn {entry.get('turn', '?')} — <span style='color:{color}'>{agent}</span>:** {entry['text']}", unsafe_allow_html=True)
...
with tab_voice:
    ...
    with col_voice2:
        ...
        text_input = st.text_area("Type your debate argument:", height=120,
        ...
        if st.button("💬 Submit to Debate", type="primary",
            ...
            if st.session_state.awaiting_human:
                ...
                st.success("✅ Your argument has been added to the debate! Return to the Arena tab.")
                st.rerun()
            else:
                st.info("No debate is currently waiting for human input. Start a debate with Human-in-the-Loop enabled.")

    st.divider()
    st.markdown("#### 🔊 Listen to Agent Outputs (TTS Simulation)")
    st.caption("In the full Sokrates system, each agent's output would be synthesized via TTS and played back here with prosodic analysis.")

    if st.session_state.debate_history:
        agent_filter = st.selectbox("Select agent to replay:", ["All"] + ["Alpha", "Beta", "Gamma", "Judge"])
        entries = [e for e in st.session_state.debate_history
                   if agent_filter == "All" or e["agent"] == agent_filter]
        for entry in entries[-3:]:  # Show last 3
            agent = entry["agent"]
            color, _ = AGENT_COLORS.get(agent, ("#aaa", ""))
            st.markdown(f"**<span style='color:{color}'>{agent}</span> (Turn {entry.get('turn','?')}):** {entry['text']}", unsafe_allow_html=True)
            # TTS placeholder
            if st.button(f"▶ Play {agent} Turn {entry.get('turn','?')}", key=f"tts_{agent}_{entry.get('turn','?')}"):
                st.info("🔊 TTS playback would stream audio here via Pipecat/ElevenLabs in the full system.")
    else:
...
with tab_rollouts:
    ...
    if "rollout_games" in st.session_state and st.session_state.rollout_games:
        ...
        st.markdown(f"**{len(games)}** rollout(s) loaded from `{st.session_state.get('rollout_file', '')}`")
        ...
        st.caption(f"✅ {successes} success · ❌ {errors} error · 📊 {len(games)} total")
        ...
        status_emoji = "✅" if r_status.get("success") else ("❌" if r_status.get("error") else "⏹")
        st.markdown(
        ...
        st.info("Enter a results JSONL file path above and click **Load Results** to browse rollouts.")

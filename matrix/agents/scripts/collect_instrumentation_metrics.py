...
def analyze_latency_breakdown(records: list[dict[str, Any]]) -> pd.DataFrame:
    ...
    for record in records:
        ...
        latency_by_metric_role: dict[tuple[str, str], float] = defaultdict(float)
...
def print_bandwidth_timeseries(
    ...
    bin_spike_times: list[list[tuple[float, float]]] = [[] for _ in range(actual_bins)]
...
def main(
    input_file: str,
    window: float = 60.0,
    extreme_mult: float = 100.0,
) -> None:
    """
    Collect and analyze agent metrics from instrumentation data.

    Args:
        input_file: Path to input JSONL file or glob pattern
            (e.g., 'results/*.jsonl', 'data/**/*.jsonl.zst')
        window: Sliding window size in seconds for bandwidth/throughput calculation
        extreme_mult: Multiplier for extreme bandwidth threshold (threshold = p90 * mult)
    """
    print(f"Loading data from {input_file}...")

    # Check if input is a glob pattern or single file
    if "*" in input_file or "?" in input_file:
        records = load_jsonl_glob(input_file)
    else:
    ...
    if not df_events.empty:
        ...
        df_throughput, throughput_role_stats, overall_tp_stats = (
            analyze_throughput_breakdown(df_events, window_seconds=window)
        )
        ...
        df_bandwidth, bw_role_stats, overall_bw_stats = analyze_network_bandwidth(
            df_events, window_seconds=window
        )
        ...
        print_bandwidth_timeseries(
            ...
            extreme_threshold_mult=extreme_mult,
        )
    else:
